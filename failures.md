...
These are not counted as semantic failures by the runner (`execution_failed=0`), but they are outstanding coverage gaps. A combined Release audit over the `102` files currently absent from `TESTED.md` selected `530,862` rows: `58,832 passed, 0 failed, 472,030 skipped`.
