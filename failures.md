...
| SHA extension instructions | `./build-release/remill-tester 3975WX/sha1nexte.txt 3975WX/sha1rnds4.txt 3975WX/sha256msg1.txt 3975WX/sha256msg2.txt 3975WX/sha256rnds2.txt --execute` | pending re-audit after `sha1msg1`/`sha1msg2` fixes; remaining rows skipped as `sha_extension_unsupported` | `sha1msg1.txt` is fixed by Remill submodule commit `a227ce9` and passes `63,904` rows; `sha1msg2.txt` is fixed by Remill submodule commit `253fd61` and passes `64,432` rows. Remaining SHA extension semantics are still unsupported. |
