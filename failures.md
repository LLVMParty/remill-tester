...
These are not counted as semantic failures by the runner (`execution_failed=0`), but they are outstanding coverage gaps. A combined Release audit over the `90` files currently absent from `TESTED.md` selected `298,631` rows: `165,202 passed, 0 failed, 133,429 skipped`.
...
| x87/FPU state unsupported | combined remaining-file audit over x87 files | `43,094 skipped`; `fpu_state_unsupported=43,094` | Requires the broader x87 stack/tag/control bridge before comparing arithmetic and data-transfer instructions. A minimal status-word bridge covers `fnstsw`, `fninit`, `fnclex`, `fincstp`, `fdecstp`, `ffree`, and `ffreep` (`583` rows passed). Remill submodule commits `6f6c240`/`04a8247` and row-level filtering now also execute canonical `fabs`/`fchs`/`fxch`/`fcmov*` rows (`493` additional rows passed) while skipping untrusted raw x87 stack encodings. |
